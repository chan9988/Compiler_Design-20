...
const char *FunctionNode::getinputtype() const{
    if (!inputtype_string_is_valid) {

        for (const auto &parameter : parameters) {
            for (const auto &var : parameter->variables()) {
                inputtype_string.append(var->getTypeCString()).append(", ");
            }
        }

        if (!parameters.empty()) {
            // remove the trailing ", "
            inputtype_string.erase(inputtype_string.end() - 2,
                                   inputtype_string.end());
        }

        inputtype_string_is_valid = true;
    }

    return inputtype_string.c_str();
}
